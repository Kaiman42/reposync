...
async function fetchRepoDetails(path) {
    return await window.go.main.App.GetRepoDetails(path);
}
...
function renderRepos() {
    ...
    filtered.forEach(repo => {
        ...
        card.onclick = () => window.go.main.App.OpenAction(repo.path, 'explorer');
...
async function removePath(path) { if (confirm('Remover?')) { await window.go.main.App.RemovePath(path); fetchBasePaths(); } }
async function addPath() { const p = prompt("Caminho:"); if (p) { await window.go.main.App.AddPath(p); fetchBasePaths(); } }
